export const getAvatarClosure = () => {
  ...
  */
  ...
  const [
    green, warmPeach, softSand, burntSienna, iceBlue, lavenderMist,
    blushPink, creamApricot, periwinkleLight
  ] = Object.values(colors);

  const bgValues = [green, warmPeach, softSand, burntSienna, iceBlue, lavenderMist,
    blushPink, creamApricot, periwinkleLight].join(',')
  const domain = `${base}/9.x/${faceType}/svg`;
  return (player, r = R) => `${domain}?radius=${r}&backgroundColor=${bgValues}&seed=${player}`
}