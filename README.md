# <font color="Gray">This is a recreation of `Organ Attack` Card Game. </font>

## This is a team effort

## Our Team

| Name               | Role             |
| ------------------ | ---------------- |
| Sirisha Dalasari   | UI Dev           |
| Nandini Dasari     | Full Stack       |
| Vivek Bhardwaj     | Full Stack       |
| Samiran Gharami    | Business Analyst |
| Nikhil Chodavarapu | Full Stack       |
| Chiranjeevi Gurram | Full Stack       |
| Adityan K          | UI Dev           |
| Shivang Singh      | Quality Analyst  |

# About our game project

- A collaborative effort by our team to make a turn based (out of turn) card
  game.

## `Organ Attack` **description**:

- player play their turn to defend and afflict damage to organs of each other .

- Non turn player can still defend themselves and can afflict damage using the
  <font color='#b42929'>_**instant cards**_</font> .

## 🌐 Try the Game

> [Play Here](https://organ-attack-leucocytes.onrender.com/)

Experience the game live in your browser! No installation needed—just gather
your friends, The version supports:

- Real-time multiplayer functionality
- Interactive game board

---
